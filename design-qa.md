...
構成：ヒーロー → 導入実績 → 漫画2ページ → 報告から共有への流れ → 既存の機能説明・料金・無料デモ。承認済みの漫画を使用し、1ページ目の見出しは「現場は回る、でも声が埋もれる」。PCは左から右へ2ページ、幅880px以下は上から下へ。画像の拡大リンクを用意する。登場人物・会話は架空とHTMLでも注記する。

| 境界 | 処理 |
| --- | --- |
| 導入実績 → 漫画 | LPの生成りから漫画の紙色へ淡い背景グラデーションで接続。見出しと本文の余白を確保 |
| 漫画1 → 漫画2 | 各ページの紙色・余白を保持。PCは間隔24px、スマホは順序を明示して縦積み。文字やコマをマスクしない |
| 漫画 → 流れの説明 | 紙色からLPの生成りへ戻す。下部に既存デモへのリンクを置く |

モーションは見出し・各ページで既存のスクロール表示を再利用。画像寸法を予約し、遅延読み込みで初期表示への影響を抑える。WebP2枚は計801,716バイト（864×1821pxを維持）。

検証済み：320/390/768/1024/1440pxで横はみ出しなし、880px以下の縦積み、PC2列、2ページ目までの画像読込・スクロール表示、画像拡大リンク、漫画CTAから無料デモへの移動、フォーム入力時の固定CTA非表示、動き停止時の両ページ表示。ブラウザconsoleエラーなし。HTMLの重複ID・内部リンク・ローカル画像・画像寸法の検査成功。既存の35テスト成功（上流送信stub、実送信なし）。JavaScript・API・フォーム項目・送信先の変更なし。
