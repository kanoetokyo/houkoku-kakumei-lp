...
- Spacing/layout: 左コピー・右会話、濃いオレンジの変化説明、交互配置の3章、料金・CTAを維持。タッチ操作44px以上。320/390/768/1024/1280/1440pxで横はみ出しなし。
- Colors/tokens: オレンジ・生成り・淡いアプリオレンジ・金色を維持。本文と背景、ボタンと文字、フォーカスリングを視認確認。
...
## オレンジ基調とiOSアプリ案内（2026年9月5日・日本時間）

ターゲット：現場の小さな気づきを改善につなげたい中小企業の経営者。目的：無料デモへの到達を保ちながら、iOSアプリの準備中を安心して伝える。基調は提供されたiOSアプリアイコンの鮮やかなオレンジ、やわらかい生成り、濃いブラウン。CTAは既存の無料デモへ統一し、App Store公開や利用開始は案内しない。

構成：ヒーロー → 導入実績 → 漫画 → 文化 → 機能3章 → iOSアプリ準備中 → 料金 → 無料デモ → FAQ。iOS案内には提供アイコンを使い、未公開のアプリ画面やダウンロードボタンは掲載しない。

| 境界 | リスク | つなぐ要素 | CSS処理 | 追加画像 | 確認 |
| --- | --- | --- | --- | --- | --- |
| 導入実績 → 漫画 | 実績帯と紙色の差 | 生成りから淡いアプリオレンジへの光 | 既存グラデーションを暖色化 | 不要 | 漫画上端 |
| 文化 → 機能 | 濃色帯から淡色への切替 | 濃いオレンジと生成り | 背景色と罫線を暖色化 | 不要 | 文化帯下端 |
| 機能3章 → iOS案内 | 情報ブロックが続く | アイコンの丸みと淡いオレンジの光 | ラジアルグラデーションと細い暖色罫線 | 提供アイコン | iOS案内上端 |
| iOS案内 → 料金 | アプリ案内と料金の役割差 | 生成りへ戻る余白 | 下端を生成りへフェード | 不要 | 料金上端 |
